// === SUPABASE DATA RESTORATION ===
var __savedBoreholes = null;
var __savedMapSettings = null;
if (window.__supabaseData) {
  if (window.__supabaseData.boreholes) {
    __savedBoreholes = window.__supabaseData.boreholes;
  }
  if (window.__supabaseData.mapSettings) {
    __savedMapSettings = window.__supabaseData.mapSettings;
  }
}
try {
...
// WGS84 to RD (inverse) â€” Benaderingsformules Schreutelkamp & Strang van Hees
...
if(data.length>0){
  ...
  document.getElementById('info').innerHTML='<span>Geen boorpunten â€” importeer via CSV/Excel of voeg handmatig toe</span>';
}
...
// Bereken center alvast â€” default Nederland midden als geen data
...
cur.on('tileerror',function(){
  st.textContent='Satelliet tiles laden niet â€” probeer een andere laag (OSM/Kaart)';
...
data.forEach(function(b){
  ...
  m.bindPopup('<b>'+b.n+'</b><br>'+b.d+'m | âŒ€'+b.dia+'mm<br>RD: '+b.x+', '+b.y);
  ms.push(m);
...
// === SUPABASE: Save map settings on move ===
map.on('moveend', function() {
  if (window.__supabaseSave) {
    window.__supabaseSave('mapSettings', { center: map.getCenter(), zoom: map.getZoom() });
  }
});

// === SUPABASE: Restore saved boreholes ===
if (__savedBoreholes && Array.isArray(__savedBoreholes) && __savedBoreholes.length > 0) {
  __savedBoreholes.forEach(function(sb) {
    var lat = sb.lat, lng = sb.lng;
    if (!lat || !lng) {
      if (sb.x && sb.y) { var ll = rd(sb.x, sb.y); lat = ll[0]; lng = ll[1]; }
      else return;
    }
    var b = {n:sb.n||'?', x:sb.x||0, y:sb.y||0, d:sb.d||0, dia:sb.dia||0, lat:lat, lng:lng, custom:true};
    data.push(b);
    addedPoints.push(b);
    var m2 = L.circleMarker([lat,lng], {
      radius: b.dia===45?8:6, fillColor: cc(b.d), color:'#fff',
      weight: b.dia===45?2.5:1.5, fillOpacity:0.9
    }).addTo(map);
    m2.bindPopup('<b>'+b.n+'</b> (opgeslagen)<br>'+b.d+'m | \u00D8'+b.dia+'mm<br>RD: '+b.x+', '+b.y);
    ms.push(m2); b._marker = m2;
    var lb2 = L.marker([lat,lng],{icon:L.divIcon({
      className:'',iconSize:[0,0],
      html:'<div style="font-size:8px;font-weight:700;color:#ff0;text-shadow:1px 1px 1px #000,-1px -1px 1px #000;transform:translate(-50%,-16px);text-align:center;white-space:nowrap">'+b.n+'</div>'
    })}).addTo(map);
    ls.push(lb2); b._label = lb2;
  });
...
// === SUPABASE: Restore saved map view ===
if (__savedMapSettings && __savedMapSettings.center) {
  var c = __savedMapSettings.center;
  map.setView([c.lat, c.lng], __savedMapSettings.zoom || 15);
}
...
window.sw=function(k,btn){
  ...
  cur.on('tileerror',function(){st.style.display='block';st.textContent='Deze tiles laden niet â€” probeer andere laag';});
...
window.toggleAdd=function(){
  ...
  if(addMode){
    btn.style.background='#1b5e20';btn.textContent='âœ“ Plaatsen...';
...
map.on('click',function(e){
  ...
  data.push(b);
  addedPoints.push(b);
  if(window.__supabaseSave) window.__supabaseSave('boreholes', addedPoints);
...
function refreshTable(){
  var h2='';data.forEach(function(b,i){
    ...
    h2+='<tr style="'+style+'"><td>'+(i+1)+'</td><td><b>'+b.n+'</b>'+(b.custom?' ðŸ†•':'')+'</td><td>'+b.x+'</td><td>'+b.y+'</td><td style="color:'+cc(b.d)+';font-weight:600">'+b.d+'m</td><td>'+diaDisp+'</td></tr>';
  });
...
window.zoekAdres=function(){
  ...
  fetch('https://api.pdok.nl/bzk/locatieserver/search/v3_1/free?q='+encodeURIComponent(q)+'&rows=1&fq=type:adres')
    ...
    .then(function(j){
      ...
      if(!m){res.textContent='Geen coÃ¶rdinaten';res.style.color='#c62828';return;}
      ...
      adresMarker=L.marker([lat,lng],{icon:L.divIcon({
        className:'',iconSize:[0,0],
        html:'<div style="font-size:20px;transform:translate(-10px,-30px)">ðŸ“</div>'
      })}).addTo(map);
      adresMarker.bindPopup('<b>'+doc.weergavenaam+'</b><br>'+rdTxt).openPopup();

      res.textContent=doc.weergavenaam+(rdTxt?' â€” '+rdTxt:'');res.style.color='#2e7d32';
    })
    .catch(function(err){
...
window.clearAll=function(){
  ...
  if(window.__supabaseSave) window.__supabaseSave('boreholes', []);
  grp=L.featureGroup(ms);
  tot=0;
...
st.textContent='Kaart geladen â€” wacht op tiles...';
...
window.exportPDF=function(){
  ...
  function continueFinishPDF(){
    ...
    // Legend below map â€” dynamisch op basis van data
    var ly=mapY+mapH+4;
...
window.toggleCSV=function(){
  ...
  btn.textContent=vis?'âœ• Sluit CSV':'ðŸ“‚ CSV Import';
};
...
function parseCSVFile(file){
  ...
  if(isExcel){
    // Excel file â€” read as ArrayBuffer and parse with SheetJS
...
function processRows(headers,rows,fileName){
      ...
      // Find column indices â€” flexible matching
      ...
      headers.forEach(function(h,i){
        ...
        // Name â€” match exact or contains patterns
        ...
        // Depth â€” fuzzy: bevat "diepte" of "depth" ergens in de header
      ...
      var detectedInfo='Kolommen: '+(colNaam>=0?'Naam='+headers[colNaam]:'âš ï¸ geen naam')+', X='+headers[colX]+', Y='+headers[colY]+(colType>=0?', Type='+headers[colType]:'')+(colDiepte>=0?', Diepte='+headers[colDiepte]:' âš ï¸ GEEN DIEPTE GEVONDEN');
      csvStatusEl.textContent=detectedInfo;
      console.log('CSV Import â€” Headers gevonden:',headers);
      console.log('CSV Import â€” Kolom indices: Naam='+colNaam+' X='+colX+' Y='+colY+' Diepte='+colDiepte+' Dia='+colDia+' Type='+colType);
      if(rows.length>0) console.log('CSV Import â€” Eerste rij:',rows[0]);
      ...
      for(var li=0;li<rows.length;li++){
        ...
        // Detect if WGS84 (lat/lng) vs RD â€” RD X is typically 0-300000, Y 300000-625000
        var rdX,rdY;
        if(xVal>1000 && yVal>1000){
          // Assume RD
          rdX=Math.round(xVal);rdY=Math.round(yVal);
        } else {
          // Assume WGS84 lat/lng â€” convert
        ...
        if(isNaN(diepte)||diepte<=0){
          // Extract depth from type name (e.g. bo30 â†’ 30m, bo200 â†’ 200m)
        ...
        var diaLabel=dia>0?('\u00D8'+dia+'mm'):'âŒ€ onbekend';
      ...
      csvStatusEl.textContent='âœ… '+imported+' punten geÃ¯mporteerd uit '+fileName+(skipped>0?' ('+skipped+' overgeslagen)':'');
}
...
window.toggleKlic=function(){
  ...
  btn.textContent=vis?'âœ• Sluit KLIC':'âš¡ KLIC';
};
...
function processKlicZip(file){
  ...
  reader.onload=function(e){
    JSZip.loadAsync(e.target.result).then(function(zip){
      ...
      if(pngPaths.length===0&&gmlFiles.length===0){
        ...
        if(matchedPgw){
          var p=Promise.all([
            ...
            if(pgwLines.length>=6){
              ...
              // We need image dimensions â€” create temp image
          ...
          // PNG without worldfile â€” try to find extent in XML
      ...
      Promise.all(promises).then(function(){
        ...
        klicStatusEl.innerHTML='âœ… '+file.name+': '+mapped+' kaartlagen'+(vectors>0?', '+vectors+' vectorlagen':'')+(noref>0?' ('+noref+' zonder georef)':'')+'<br><span style="font-size:11px;color:#555">Lagen: '+summaryParts.join(' | ')+'</span>';
...
// Detecteer KLIC thema/type â€” meerdere detectiemethodes
function detectKlicThema(path,text){
  ...
  // === 5. Netbeheerder namen â†’ type mapping ===
...
// === IMKL v2 NETWERK TYPE â†’ KLEUR MAPPING ===
var KLIC_NET_COLORS={
...
function parseKlicXML(text,path,klicEntry){
  try{
    ...
    // === 3. UTILITYLINKS â€” leidingen per netwerk-type, gegroepeerd op kleur ===
    ...
    utilLinks.forEach(function(ul){
      ...
      // Extract geometry: centrelineGeometry > LineString > posList
      var lineStrings=findAllByLocalName(ul,'LineString');
      lineStrings.forEach(function(ls){
        var pls=findAllByLocalName(ls,'posList');
        pls.forEach(function(pl){
          var latlngs=parseRDCoords(pl.textContent);
          if(latlngs.length>=2) netGroups[groupKey].lines.push(latlngs);
        });
      });
      // Also check for Curve elements
      var curves=findAllByLocalName(ul,'Curve');
      curves.forEach(function(c){
        var pls=findAllByLocalName(c,'posList');
        pls.forEach(function(pl){
          var latlngs=parseRDCoords(pl.textContent);
          if(latlngs.length>=2) netGroups[groupKey].lines.push(latlngs);
        });
    ...
    Object.keys(netGroups).forEach(function(key){
      ...
      console.log('KLIC: Laag "'+grp.label+'" â†’ '+grp.lines.length+' segmenten, kleur: '+grp.color);
...
function updateKlicPanel(){
  ...
  klicLayers.forEach(function(kl,ki){
    ...
    kl.layers.forEach(function(layer,li){
      ...
      var icon=layer.type==='polygon'?'â—†':layer.type==='line'?'â”':layer.type==='png'?'ðŸ—ºï¸':'ðŸ“„';
...
window.toggleMeasure=function(){
  ...
  if(measureMode){
    btn.style.background='#4a148c';btn.textContent='ðŸ“ Meten... (klik)';
    map.getContainer().style.cursor='crosshair';
    clearMeasure();
    showMeasurePanel(true);
  } else {
    btn.style.background='#6a1b9a';btn.textContent='ðŸ“ Meten';
    map.getContainer().style.cursor='';
...
function showMeasurePanel(show){
  ...
  if(!existing){
    ...
    panel.innerHTML='ðŸ“ Klik op de kaart om afstanden te meten &nbsp;|&nbsp; '+
      '<span id="measureInfo" style="font-weight:600;color:#4a148c">0 punten â€” 0 m</span> &nbsp;|&nbsp; '+
      '<button onclick="undoMeasure()" style="padding:3px 10px;border:1px solid #6a1b9a;border-radius:4px;font-size:11px;cursor:pointer;background:#fff;font-weight:600;color:#6a1b9a">â†© Laatste punt</button> '+
      '<button onclick="clearMeasure()" style="padding:3px 10px;border:1px solid #c62828;border-radius:4px;font-size:11px;cursor:pointer;background:#fff;font-weight:600;color:#c62828">ðŸ—‘ï¸ Wis meting</button>';
    // Insert before map
...
function updateMeasureInfo(){
  ...
  if(n<2){
    el.textContent=n+' punt'+(n!==1?'en':'')+' â€” klik voor meer';
  } else {
    var txt=n+' punten â€” ';
